  ...
  async render() {
    ...
    try {
      ...
      try {
        ...
        try { this.statsEl = el.createDiv("folio-stats"); } catch {}
      }
...
class NewBookModal extends Modal {
  ...
  async onOpen() {
    ...
    // Template row
    makeDivider();
    const tplRow = contentEl.createDiv({ cls: 'folio-modal-row' });
    const tplLeft = tplRow.createDiv({ cls: 'folio-modal-left' });
    tplLeft.createEl('div', { text: 'Template', cls: 'folio-modal-row-title' });
    tplLeft.createEl('div', { text: 'Please select a book template', cls: 'folio-modal-row-sub' });
    const tplRight = tplRow.createDiv({ cls: 'folio-modal-right' });
    const tplSelect = tplRight.createEl('select', { cls: 'folio-template-select' });
