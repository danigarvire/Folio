...
<html>
  ...
  loadScreenplaySnippetCss() {
    try {
      ...
      const candidates = [
        import_path.default.join(__dirname, "PRO Screenwriting Snippet.css"),
        basePath ? import_path.default.join(basePath, ".obsidian", "snippets", "PRO Screenwriting Snippet.css") : null,
        basePath ? import_path.default.join(basePath, "PRO Screenwriting Snippet.css") : null
      ].filter(Boolean);
...
module.exports = class FolioPlugin extends Plugin {
  ...
  async createNextChapterFile(folder) {
    ...
    try {
      ...
      const path2 = `${folderObj.path}/${fileName}`;
      if (this.app.vault.getAbstractFileByPath(path2))
        return;
      await this.app.vault.create(path2, "");
      try {
        const book = this.booksIndex.find((b) => path2.startsWith(b.path));
        if (book)
          await this.syncChapterStatsBaseline(book);
      } catch (e) {
  ...
  async deleteFolderRecursive(path2) {
    try {
      ...
      console.warn("deleteFolderRecursive (trash) failed", path2, e);
    }
  }
  ...
  async waitForFolderSync(path2, retries = 20) {
